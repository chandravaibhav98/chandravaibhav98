...
let carouselIndices = {
	keyProjectsCarousel: 0,
	projectsCarousel: 0,
	skillsCarousel: 0,
	technicalSkillsCarousel: 0,
};

function loadData() {
	loadCarouselData(keyProjects, "keyProjectsCarousel");
	loadCarouselData(projects, "projectsCarousel");
	loadCarouselData(skills, "skillsCarousel");
	loadCarouselData(technicalSkills, "technicalSkillsCarousel");
}

function loadCarouselData(items, carouselId) {
	const carousel = document.getElementById(carouselId);
	items.forEach((item) => {
		const card = createCard(item);
		carousel.appendChild(card);
	});
	showCard(carouselId, 0); // Show the first card in the carousel
}

function showCard(carouselId, index) {
	const carousel = document.getElementById(carouselId);
	const cards = carousel.children;

	// Update the index within the valid range
	if (index >= cards.length) {
		carouselIndices[carouselId] = 0;
	} else if (index < 0) {
		carouselIndices[carouselId] = cards.length - 1;
	} else {
		carouselIndices[carouselId] = index;
	}

	// Calculate the offset to show the current card
	const offset = -carouselIndices[carouselId] * 100;
	carousel.style.transform = `translateX(${offset}%)`;
}

function prevCard(carouselId) {
	const currentIndex = carouselIndices[carouselId];
	showCard(carouselId, currentIndex - 1);
}

function nextCard(carouselId) {
	const currentIndex = carouselIndices[carouselId];
	showCard(carouselId, currentIndex + 1);
}
...
// window.filterProjects = function (language) {
// 	const cards = document.querySelectorAll(".card");
// 	let firstVisibleCardIndex = null;
...
// 	// Show the first visible card in the carousel
// 	if (firstVisibleCardIndex !== null) {
// 		const carouselId = cards[firstVisibleCardIndex].closest(".carousel").id;
// 		showCard(carouselId, firstVisibleCardIndex);
// 	} else {
// 		// If no cards match, optionally hide the carousel or display a message
